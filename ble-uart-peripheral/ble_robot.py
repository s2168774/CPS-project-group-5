...
CONTROLLABLE_ROBOT_SERVICE_UUID = 'a7ead335-61e5-4d23-a4ce-bd0a956d5952'
...
class ModeCharacteristic(Characteristic):
    ...
    def WriteValue(self, value, options):
        print('recieved mode: {}'.format(bytearray(value).hex()))
...
class TargetCharacteristic(Characteristic):
    ...
    def WriteValue(self, value, options):
        print('recieved target: {}'.format(bytearray(value).hex()))
...
class RobotAdvertisement(Advertisement):
    def __init__(self, bus, index):
        Advertisement.__init__(self, bus, index, 'peripheral')
        self.add_service_uuid(CONTROLLABLE_ROBOT_SERVICE_UUID)
        self.add_local_name(LOCAL_NAME)
        self.include_tx_power = True
